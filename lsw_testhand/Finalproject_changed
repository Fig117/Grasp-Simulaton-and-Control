...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and time.time() - start < 1000:
        ...
        if np.abs(distance_to_target[0]) > error and np.abs(distance_to_target[1]) > error and np.abs(distance_to_target[2]) > error and init_flag == False:
          ...
          ff_tip_control_signals = compute_control_signals(1,0.2,0.02,ff_tip_distance_to_target)
          mf_tip_control_signals = compute_control_signals(1,0.2,0.02,mf_tip_distance_to_target)
          rf_tip_control_signals = compute_control_signals(1,0.2,0.02,rf_tip_distance_to_target)
