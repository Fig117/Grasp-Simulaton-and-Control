...
def compute_force_feedback_term(current_force, desired_force_mag=1.32, gain=0.012):
    current_mag = np.linalg.norm(current_force)
    error = desired_force_mag - current_mag
    if current_mag > 1e-5:
        direction = current_force / current_mag
    else:
        direction = np.zeros(3)
    return gain * error * direction
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running() and time.time() - start < 1000:
        ...
        ff_force = data.sensor('ff_force_sensor').data.copy()
        mf_force = data.sensor('mf_force_sensor').data.copy()
        rf_force = data.sensor('rf_force_sensor').data.copy()
        th_force = data.sensor('th_force_sensor').data.copy()
        ...
        if np.abs(distance_to_target[0]) > error and np.abs(distance_to_target[1]) > error and np.abs(distance_to_target[2]) > error and init_flag == False:
          ...
          # Calculate the distance to the target
          ff_tip_distance_to_target = (ff_tip_target - ff_tip_current_pos) + compute_force_feedback_term(ff_force)
          mf_tip_distance_to_target = (mf_tip_target - mf_tip_current_pos) + compute_force_feedback_term(mf_force)
          rf_tip_distance_to_target = (rf_tip_target - rf_tip_current_pos) + compute_force_feedback_term(rf_force)
          th_tip_distance_to_target = (th_tip_target - th_tip_current_pos) + compute_force_feedback_term(th_force)
